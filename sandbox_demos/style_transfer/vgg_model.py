...
def _conv2d_relu(vgg_layers, prev_layer, layer, layer_name):
    ...
    # converts to TF tensors
    W = tf.constant(W)
    b = tf.constant(b)
    # returns tensor after going through convolution
...
def _avgpool(prev_layer):
    ...
    # pools over 2x2 window
    return tf.nn.avg_pool(value=prev_layer, ksize=[1,2,2,1], strides=[1,1,1,1], padding='SAME')
